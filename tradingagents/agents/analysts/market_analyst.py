        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    "[如果工具结果包含“筹码峰分析”，必须解读主筹码峰、70%/90%成本区间、成本集中度、区间重合度、获利盘/压力盘、底峰保留、量能变化和筹码迁移方向]\n"
                    "[必须基于筹码峰变化判断主力行为：建仓吸筹、洗盘、锁仓、拉升中继、派发/抛售或信号不明]\n"
                    "\n"
                    "## 📉 价格趋势分析\n"
                    ...
                    "- 如果工具结果包含“筹码峰分析”，不要忽略它；必须说明它对支撑位、压力位和主力动向的含义\n"
...
[如果工具数据中包含“5. 筹码峰分析”，必须将该小节放在布林带之后，并保留原始筹码峰SVG图]
[分析筹码峰指标，包括：]
- 主筹码峰、次筹码峰和峰型
- 70%/90%成本区间、成本集中度、区间重合度
- 获利盘、压力盘、底部筹码峰保留率
- 主峰迁移、量能变化和主力行为判断（建仓吸筹/洗盘/锁仓/拉升中继/派发抛售/信号不明）
- 筹码峰只能作为辅助判断，需结合成交量、K线、基本面和市场环境
